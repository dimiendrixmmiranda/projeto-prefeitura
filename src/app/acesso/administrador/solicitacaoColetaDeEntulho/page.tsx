...
export default function Page() {
    const { solicitacaoColetaDeEntulho, loading } = useSolicitacaoColetaDeEntulho()
    const [visible, setVisible] = useState(false);
    const [solicitacaoAtual, setSolicitacaoAtual] = useState<SolicitacaoColetaDeEntulho | null>(null)

    function selecionarSolicitacaoAtual(solicitacao: SolicitacaoColetaDeEntulho) {
    ...
    async function confirmarSolicitacao(id: string) {
        try {
            const docRef = doc(db, "listaSolicitacaoColetaDeEntulho", id);
    ...
    return (
        <Template>
            <div className="p-4 flex flex-col gap-2 md:min-h-[72vh] 2xl:min-h-[73.5vh] xl:p-10">
                <h2 className="uppercase font-black text-2xl text-center text-verde-escuro">Painel de solicitações de Coleta de Entulho</h2>
                            ...
                            <table className="w-full border border-gray-300 text-sm text-left">
                                <thead className="bg-gray-200 text-gray-700">
                                    <tr>
                                        ...
                                        <th className="p-2 border border-gray-400 ">Tipo de Entulho</th>
                                ...
                                <tbody>
                                    {solicitacaoColetaDeEntulho.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-100">
                                            ...
                                            <td className="p-2 border truncate text-center">{item.tipoDeEntulho}</td>
