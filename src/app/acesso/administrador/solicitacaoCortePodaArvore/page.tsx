...
import { useSolicitacoesCortePodaDeArvores } from "@/lib/hooks/useSolicitacoesCortePodaDeArvores";
...
export default function Page() {
    const { solicitacaoCorteEPodaDeArvore, loading } = useSolicitacoesCortePodaDeArvores()
    const [visible, setVisible] = useState(false);
    const [solicitacaoAtual, setSolicitacaoAtual] = useState<SolicitacaoCortePodaArvore | null>(null)

    function selecionarSolicitacaoAtual(solicitacao: SolicitacaoCortePodaArvore) {
    ...
    async function confirmarSolicitacao(id: string) {
        try {
            const docRef = doc(db, "listaSolicitacoesCorteEPodaDeArvores", id);
    ...
    return (
        <Template>
            <div className="p-4 flex flex-col gap-2 md:min-h-[72vh] 2xl:min-h-[73.5vh] xl:p-10">
                <h2 className="uppercase font-black text-2xl text-center text-verde-escuro">Painel de solicitações de Corte e Poda de Arvore</h2>
                            ...
                            <table className="w-full border border-gray-300 text-sm text-left">
                                <thead className="bg-gray-200 text-gray-700">
                                    <tr>
                                        ...
                                        <th className="p-2 border border-gray-400 ">CPF</th>
                                        ...
                                        <th className="p-2 border border-gray-400 ">Altura</th>
                                        <th className="p-2 border border-gray-400 ">Motivo</th>
                                        <th className="p-2 border border-gray-400 ">Serviço</th>
                                ...
                                <tbody>
                                    {solicitacaoCorteEPodaDeArvore.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-100">
                                            ...
                                            <td className="p-2 border truncate text-center">{item.alturaArvore}</td>
                                            <td className="p-2 border truncate text-center">{item.motivoDaSolicitacao}</td>
                                            <td className="p-2 border truncate text-center">{item.tipoDeServico}</td>
                                ...
                                <table className="w-full border border-gray-300 text-sm text-left">
                                    ...
                                    <tbody>
                                        <tr key={solicitacaoAtual.id} className="hover:bg-gray-100">
                                            ...
                                            <td className="p-2 border truncate text-center">{solicitacaoAtual.alturaArvore}</td>
                                            <td className="p-2 border truncate text-center">{solicitacaoAtual.motivoDaSolicitacao}</td>
                                            <td className="p-2 border truncate text-center">{solicitacaoAtual.tipoDeServico}</td>
                                            <td className="p-2 border truncate text-center">
