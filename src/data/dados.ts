export const menuCabecalho = [
    {
        tituloCabecalho: 'Governo Atual',
        subtitulos: [
            'Conheça o Prefeito',
            'Composição da Câmara',
            'Galeria de Prefeitos',
        ]
    },
    {
        tituloCabecalho: 'Secretarias',
        subtitulos: [
            'Secretaria da Educação',
            'Secretaria da Saúde',
            'Secretaria da Agricultura',
            'Secretaria de Infraestrutura Urbana',
            'Secretaria de Divisão de Cultura',
            'Secretaria de Esporte, Lazer e Turismo',
            'Secretaria de Desenvolvimento Econômico e Inovação',
            'Assistência Social',
            'Divisão de Engenharia',
            'Administração e Fazenda',
            'Gabinente',
            'Assessoria Júridica do Gabinete',
            'Assessoria Júridica do Gabinete',
        ]
    },
    {
        tituloCabecalho: 'O Município',
        subtitulos: [
            'Aspectos Educacionais',
            'Aspectos Geográficos',
            'Aspectos Históricos do Município',
            'Aspectos Populacionais e Socioêconomicos',
            'Aspectos Socioculturais',
            'Cultura e Eventos',
            'Demografia',
            'Etimologia',
            'Símbolos Municipais',
        ]
    },
    {
        tituloCabecalho: 'Serviços',
        subtitulos: [
            'Transparência',
            'Licitações',
            'Legislação',
            'Concursos',
            'Oleríte',
            'Nota do Produtor Rural',
            'Obras',
            'Coleta de Lixo',
            'Coleta de Lixo Reciclável',
            'Corte e Poda de Árvores',
            'Prefeitura',
            'Câmara Municipal',
            'Postos De Saúde',
            'Hospital Municipal',
            'Samu',
            'Escolas Municipais',
            'Creches e Pré-Escolas',
            'Delegacia',
            'Sanepar',
            'Copel',
            'Fórum',
            'Fórum Eleitoral',
        ]
    },
    {
        tituloCabecalho: 'Imprensa',
        subtitulos: [
            'Fale Conosco',
            'Fale com o Prefeito',
            'Ouvidoria',
            'Controladoria',
        ]
    },
    {
        tituloCabecalho: 'Comércio',
        subtitulos: [
            'Guia Comercial',
            'Casdastre seu Comércio',
        ]
    },

]

export const menuLateral = [
    'Conheça o Prefeito',
    'Composição da Câmara',
    'Secretaria da Educação',
    'Secretaria da Saúde',
    'Secretaria de Infraestrutura Urbana',
    'Secretaria de Desenvolvimento Econômico e Inovação',
    'Oleríte',
    'Nota do Produtor Rural',
    'Transparência',
    'Licitações',
    'Prefeitura',
    'Câmara Municipal',
    'Postos de Saúde',
    'Hospital Municipal',
    'Delegacia',
    'Sanepar',
    'Samu'
]

export const aniversariantes = [
    {
        nome: 'leandro donizete',
        urlImg: '/avatar-homem.png',
        dataNascimento: '15-09', 
        anoNascimento: '1998',
        anosDeServico: 8
    },
    {
        nome: 'joyce silva',
        urlImg: '/avatar-mulher.png',
        dataNascimento: '16-09', 
        anoNascimento: '1998',
        anosDeServico: 4
    },
    {
        nome: 'leandro capixaba',
        urlImg: '/',
        dataNascimento: '15-11', 
        anoNascimento: '1992',
        anosDeServico: 2
    },
];
